...
import {IJoeRouter} from "./interfaces/dex/IJoeRouter.sol";
import {MarketplaceEventsAndErrors} from "./interfaces/MarketplaceEventsAndErrors.sol";
import {IWETH9 as IWAVAX} from "./interfaces/IWETH9.sol";
...
contract Layer3AVAX is IStargateReceiver, Ownable, MarketplaceEventsAndErrors {
    ...
    constructor(
        ...
        dexRouter = IJoeRouter(_dexRouter);
        USDC = IERC20(_usdcAddress);
        wrappedNative = IWAVAX(_wrappedNative);
    }
    ...
    function _executeBuy(
        ...
        uint256[] memory amountsOut = dexRouter.swapExactTokensForAVAX(
            amount,
            0,
            path,
            address(this),
        ...
        if (amountsOut[1] < listingPriceWithTolerance) {
            // refund the buyer in full with native token
            _unwrapNative(amountsOut[1], buyer);
            ...
            uint256 sellerFee = (amountsOut[1] * fee) / intConstant;
            amountsOut[1] = amountsOut[1] - sellerFee;

            // pay seller, transfer owner, delist nft
            // TODO: handle potential error case
            _unwrapNative(amountsOut[1], listing.lister); //pay the seller
    ...
    function _swapForPurchase(uint256 amount) internal returns (uint256) {
        ...
        uint256[] memory amountsOut = dexRouter.swapExactAVAXForTokens{
            value: amount
